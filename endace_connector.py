...
class EndaceConnector(BaseConnector):
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Unable to parse JSON response. Error: {err_msg}"), None)
    ...
    def _process_content_response(self, response, action_result):
        ...
        message = f"Error from server. Status code: {status}"
    ...
    def _make_rest_call(self, endpoint, action_result, headers=None, params=None, data=None, method="get"):
        ...
        try:
            r = request_func(url, auth=(self.username, self.password), json=data, headers=headers, verify=self.verify_cert, params=params)
            ...
            err_msg = self._get_error_message_from_exception(e)
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Error Connecting to server. Details: {err_msg}"), resp_json)
    ...
    def _handle_test_connectivity(self, param):
        ...
        if "links" not in response:
            ...
            self.save_progress("Test Connectivity Failed")
            return action_result.set_status(phantom.APP_ERROR, f"Unexpected response: {response}")
    ...
    def _move_file_to_vault(self, container_id, file_size, type_str, local_file_path, action_result):
        ...
        try:
            ...
            err_msg = self._get_error_message_from_exception(e)
            return RetVal(
                action_result.set_status(phantom.APP_ERROR, f"Unable to get Vault item details from Phantom. Details: {err_msg}"), None
            )
        ...
        if success:
            ...
            if success:
                self.send_progress(f"Success adding file to Vault. Vault ID: {vault_id}")
    ...
    def _handle_get_pcap(self, param):
        ...
        if state != "COMPLETED":
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Error downloading file. {state}")
        ...
        try:
            ...
            err_msg = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error creating file. Error Details: {err_msg}")
        ...
        try:
            ...
            err_msg = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Unable to get Vault item details from Phantom. Details: {err_msg}")
        ...
        try:
            ...
            err_msg = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error details: {err_msg}")

        if not vault_details:
            vault_ret_val, vault_details = self._move_file_to_vault(container_id, os.path.getsize(file_path), "pcap", file_path, action_result)
        ...
        message = f"PCAP downloaded to Vault: {vault_details[phantom.APP_JSON_VAULT_ID]}"
    ...
    def _handle_run_query(self, param):
        ...
        # Add required fields.
        parameters = {"host1": param["host1"], "host2": param["host2"], "protocol": param["protocol"]}
        ...
        if not int(self.max_pcap_size) == 0:  # if set to 0, ignore max_byte_count
            if int(byte_count) > int(self.max_pcap_size):
                message = f"Exceeded maximum pcap size. {byte_count} > {self.max_pcap_size}"
                return action_result.set_status(phantom.APP_ERROR, message)
            if byte_count == -1:
    ...
    def _validate_rfc3339(self, date_time):
        ...
        rfc3339 = rf"^{date}[Tt]{time}{offset}$"
    ...
    def initialize(self):
        ...
        self._base_url = f"https://{self.server}/api/v5/"
